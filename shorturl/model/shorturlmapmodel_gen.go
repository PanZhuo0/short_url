...
var (
	...
	cacheGoShortUrlMapIdPrefix   = "cache:go:shortUrlMap:id:"
	cacheGoShortUrlMapMd5Prefix  = "cache:go:shortUrlMap:md5:"
	cacheGoShortUrlMapSurlPrefix = "cache:go:shortUrlMap:surl:"
)
...
func (m *defaultShortUrlMapModel) Delete(ctx context.Context, id uint64) error {
	...
	goShortUrlMapIdKey := fmt.Sprintf("%s%v", cacheGoShortUrlMapIdPrefix, id)
	goShortUrlMapMd5Key := fmt.Sprintf("%s%v", cacheGoShortUrlMapMd5Prefix, data.Md5)
	goShortUrlMapSurlKey := fmt.Sprintf("%s%v", cacheGoShortUrlMapSurlPrefix, data.Surl)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, goShortUrlMapIdKey, goShortUrlMapMd5Key, goShortUrlMapSurlKey)
	return err
}
...
func (m *defaultShortUrlMapModel) FindOne(ctx context.Context, id uint64) (*ShortUrlMap, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, goShortUrlMapIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultShortUrlMapModel) FindOneByMd5(ctx context.Context, md5 sql.NullString) (*ShortUrlMap, error) {
	goShortUrlMapMd5Key := fmt.Sprintf("%s%v", cacheGoShortUrlMapMd5Prefix, md5)
	var resp ShortUrlMap
	err := m.QueryRowIndexCtx(ctx, &resp, goShortUrlMapMd5Key, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultShortUrlMapModel) Insert(ctx context.Context, data *ShortUrlMap) (sql.Result, error) {
	goShortUrlMapIdKey := fmt.Sprintf("%s%v", cacheGoShortUrlMapIdPrefix, data.Id)
	goShortUrlMapMd5Key := fmt.Sprintf("%s%v", cacheGoShortUrlMapMd5Prefix, data.Md5)
	goShortUrlMapSurlKey := fmt.Sprintf("%s%v", cacheGoShortUrlMapSurlPrefix, data.Surl)
...
func (m *defaultShortUrlMapModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheGoShortUrlMapIdPrefix, primary)
}
